...
```html
<link rel="stylesheet" href="assets/css/style.css?v=03acfa078d">
```

コミットのたびに `.githooks/pre-commit` が `tools/stamp-assets.sh` を呼び、
この値を自動で計算し直します。URL が変われば必ず新しいファイルが読み込まれるので、
閲覧者にスーパーリロードをお願いする必要はありません。

値はファイルの中身から作っているので、**中身が変わったときだけ**変わります。
無駄な再ダウンロードも、意味のない差分も出ません。

リポジトリを新しく clone したときは、一度だけ次を実行してフックを有効にしてください。
